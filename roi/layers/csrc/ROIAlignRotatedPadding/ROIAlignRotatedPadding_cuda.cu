...
// Note: this implementation originates from the Caffe2 RoIAlignRotatedPadding Op
...
// works, if any, using the "legacy" RoIAlignRotatedPadding Op.
...
template <typename T>
__global__ void RoIAlignRotatedPaddingForward(
    const int nthreads,
    const T* input,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    // RoIAlignRotatedPadding supports align == true, i.e., continuous coordinate
...
template <typename T>
__global__ void RoIAlignRotatedPaddingBackwardFeature(
    const int nthreads,
    const T* top_diff,
    const int num_rois,
    const T spatial_scale,
...
} // RoIAlignRotatedPaddingBackward
...
at::Tensor RoIAlignRotatedPadding_forward_cuda(
  ...
  at::CheckedFrom c = "RoIAlignRotatedPadding_forward_cuda";
  ...
  AT_DISPATCH_FLOATING_TYPES(
      input.scalar_type(), "RoIAlignRotatedPadding_forward", [&] {
        RoIAlignRotatedPaddingForward<scalar_t><<<grid, block, 0, stream>>>(
            output_size,
...
at::Tensor RoIAlignRotatedPadding_backward_cuda(
  ...
  AT_DISPATCH_FLOATING_TYPES(
      grad.scalar_type(), "RoIAlignRotatedPadding_backward", [&] {
        RoIAlignRotatedPaddingBackwardFeature<scalar_t><<<grid, block, 0, stream>>>(
            grad.numel(),
