...
#include "ROIAlignRotatedPadding.h"
...
template <typename T>
void ROIAlignRotatedPaddingForward(
    const int nthreads,
    const T* input,
...
//    T bin_size_w = static_cast<T>(roi_width) / static_cast<T>(pooled_width);
    ...
    int roi_bin_grid_w =
        (sampling_ratio > 0) ? sampling_ratio : ceil(roi_width / pooled_width1);
    ...
    std::vector<PreCalc<T>> pre_calc(
        roi_bin_grid_h * roi_bin_grid_w * pooled_width1 * pooled_height);
    ...
    for (int c = 0; c < channels; c++) {
      ...
      for (int ph = 0; ph < pooled_height; ph++) {
        for (int pw = 0; pw < pooled_width1; pw++) {
...
template <typename T>
void ROIAlignRotatedPaddingBackward(
    const int nthreads,
    const T* grad_output,
    const T& spatial_scale,
    const int channels,
...
at::Tensor ROIAlignRotatedPadding_forward_cpu(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      input.type(), "ROIAlignRotatedPadding_forward", [&] {
        ROIAlignRotatedPaddingForward<scalar_t>(
            output_size,
...
at::Tensor ROIAlignRotatedPadding_backward_cpu(
  ...
  at::CheckedFrom c = "ROIAlignRotatedPadding_backward_cpu";
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      grad.type(), "ROIAlignRotatedPadding_forward", [&] {
        ROIAlignRotatedPaddingBackward<scalar_t>(
            grad.numel(),
